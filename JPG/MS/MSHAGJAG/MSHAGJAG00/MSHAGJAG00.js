    ...
    with (Form1) {
        ...
        if (intKbn == 3) {
            ...
            for (i = 1; i <= 100; i++) { // 1～100
                ...
                //グループコード
                var objHANJIGYOSYANM = document.getElementById("txtHANJIGYOSYANM_" + i);
                var strHANJIGYOSYANM = objHANJIGYOSYANM.value.replace(/(^\s+)|(\s+$)/g, "");
                //備考
                ...
                if ((objTARGET.checked == true)
                    ...
                    //販売事業社名(必須チェック)
                    if (strHANJIGYOSYANM.length == 0) {
                        alert("販売事業社名は必須入力です");
                        objHANJIGYOSYANM.focus();
                        return false;
                    }
                    ...
                    //販売事業者名(レングスチェック)
                    if (fncGetByte(strHANJIGYOSYANM) > 60) {
                        alert("販売事業者名は全角30文字以内で入力して下さい");
                        objHANJIGYOSYANM.focus();
                        return false;
                    }
                    //備考(レングスチェック)
...
function fncPop(strId, strOptwidth) {
    ...
    var nowday = new Date();
    var name = "MSHAGJAG00_" + nowday.getFullYear() + nowday.getMonth() + nowday.getDate() + nowday.getHours() + nowday.getMinutes() + nowday.getSeconds(); //2014/10/02 T.Ono add 2014改善開発 No20
