...
' 2010/04/28 T.Watabe クライアント、ＪＡ毎にファイルを２つまで添付できるように変更
' 2011/04/14 T.Watabe 自動FAX複数メール送信に対応する為に、JA連絡担当者毎にメールアドレスを設定できるように変更
' 2011/11/08 H.Uema   JA注意事項を設定できるように改修
' 2011/11/29 H.Uema   FAX不要区分(ｸﾗｲｱﾝﾄ)を設定できるように修正
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If MyBase.IsPostBack = False Then
            ...
            ' 203/07/09 T.Ono add 沖縄では、tabを表示しない（注意事項を表示しない）
        ...
        Else
            ...
            '2015/02/18 T.Ono del 2014改善開発 No15
            '' 2013/07/31 T.Ono add 初回以外のロード時に、JA支所ｺｰﾄﾞが未選択なら表示を変える
            'If txtCODE.Text = "" OrElse txtCODE.Text = " " Then
            '    lblpre.Text = "クライアント注意事項"
            '    strMsg.Append("Form1.document.getElementById('tblFAX_Default').style.display = 'none';") '2013/12/13 T.Ono add 監視改善2013
            'Else
            '    lblpre.Text = "JA注意事項"
            '    strMsg.Append("Form1.document.getElementById('tblFAX_Default').style.display = 'block';") '2013/12/13 T.Ono add 監視改善2013
            'End If
    ...
    Private Sub fncIni_statebf()
        ...
        'Dim i As Integer
        'For i = 1 To 10
        '    objTANCD = CType(FindControl("txtTANCD_" & CStr(i)), System.Web.UI.WebControls.TextBox) 'コントロール名を探し出し、型変換
        '    objTANCD.ReadOnly = False
        '    objTANCD.CssClass = "c-k"
        '    objTANCD.BackColor = Nothing            'ブラウザのColor制御が不安定な為Color設定を行う
        '    'objTANCD.TabIndex = 4
        'Next
    ...
    Private Sub fncIni_stateaf()
        ...
        '    objTANCD.BackColor = System.Drawing.Color.Gainsboro            'ブラウザのColor制御が不安定な為Color設定を行う
        '    'objTANCD.TabIndex = -1
        'Next
    ...
    Private Sub fncIni_notkey()
        ...
        Dim objCopy As System.Web.UI.WebControls.CheckBox '2015/02/24 T.Ono add 2014改善開発 No15
        ...
        For i = 1 To 30
            objCopy = CType(FindControl("chkCopy_" & CStr(i)), System.Web.UI.WebControls.CheckBox) '2015/02/24 T.Ono add 2014改善開発 No15
            ...
            objCopy.Checked = False '2015/02/24 T.Ono add 2014改善開発 No15
            objEDT_DT.Value = ""
            objDISP_NO.Value = CStr(i) '機械的に番号を付ける
            'objTANCD.Text = CStr(i)  '機械的に番号を付ける ' 2010/04/14 T.Watabe edit
        ...
        '2015/02/24 T.Ono add 2014改善開発 No15
    ...
    '******************************************************************************
    '*　概　要：絞込み条件を渡すプロパティ
    '*　備　考：抽出条件２の値を渡す　2015/02/18 T.Ono add 2014改善開発 No15
    '******************************************************************************
    Public ReadOnly Property pCode2() As String
        Get
            ...
            ElseIf hdnPopcrtl.Value = "2" Then      '//コード一覧
                strRec = ""
            ElseIf hdnPopcrtl.Value = "3" Then      '//登録済み一覧
    ...
    Public ReadOnly Property pBackCode() As String
        Get
            ...
            ElseIf hdnPopcrtl.Value = "2" Then
                strRec = "hdnCODE"
            ElseIf hdnPopcrtl.Value = "3" Then      '//登録済み一覧    2015/02/18 T.Ono add 2014改善開発 No15
                '「005003:51021180～51021200」の形式で入るため、ここの値は使えないが
                '指定しないと動かないので、とりあえず指定。実際はpBackCode2の値を使用する。
                strRec = "hdnCODE"
            Else
                strRec = ""
            End If
    ...
    Public ReadOnly Property pBackName2() As String
        Get
            ...
            ElseIf hdnPopcrtl.Value = "2" Then
                strRec = ""
            ElseIf hdnPopcrtl.Value = "3" Then      '//登録済み一覧    2015/02/18 T.Ono add 2014改善開発 No15
                strRec = "hdnUSER_CD_TEMP"          '//お客様コードFrom,お客様コードTo
            Else
                strRec = ""
            End If
            Return strRec
    ...
    Private Function fncbtnKensaku_ClickEvent(ByVal pstrKBN As String) As String
        ...
        Try
            ...
            Else
                ...
                Dim strCboIndex As String                                   ' 2013/07/04 T.Ono add コンボボックス選択インデックス
                ...
                For intRow = 0 To dbData.Tables(0).Rows.Count - 1
                    ...
                    'If intRow >= 10 Then Exit For '10件以上は処理抜け 2010/04/12 T.Watabe edit
                    ...
                    For i = intRow + 1 To 30
                        ...
                        ElseIf IsNumeric(sDispNo) = False Then
                            sDispNo = "0"
                        End If
                        If CInt(sDispNo) <= i Then '表示番号が空か(設定されていない場合)、ループ中の回数より低い場合(同番で登録されている場合対策)
                            ...
                            If "01" = objTANCD.Text Then
                                ...
                                'FAX不要フラグ(ｸﾗｲｱﾝﾄ)
                                hdnFAXKURA_MOTO.Value = Convert.ToString(dbData.Tables(0).Rows(intRow).Item("FAXKURAKBN")) '2015/02/18 T.Ono add 2014改善開発 №15
                                ...
                                'FAX不要フラグ(JA)
                                hdnFAXJA_MOTO.Value = Convert.ToString(dbData.Tables(0).Rows(intRow).Item("FAXKBN")) '2015/02/18 T.Ono add 2014改善開発 №15
    ...
    Private Function fncbtnJikkou_ClickEvent(ByVal pstrKBN As String) As String
        ...
        Select Case strRec
            Case "OK"
                ...
                ElseIf pstrKBN = "4" Then '4:削除
                    Call fncIni_date()
                    ' 2013/07/10 T.Ono add コンボボックスの初期化
                    fncCombo_Create_AUTOKBN()
                    fncCombo_Create_AUTOZEROFLG()
                Else
                    Call fncIni_date()
        ...
        ' 2013/07/11 T.Ono add エラーの場合は、コンボボックスの初期化と、入力値のセットを行う
    ...
    Private Function fncDataSelect(ByVal pintkbn As Integer) As DataSet
        ...
        If pintkbn = 0 Then
            ...
            strSQL.Append("TA.AUTO_MAIL_PASS, ")   '自動FAX添付ﾌｧｲﾙﾊﾟｽﾜｰﾄﾞ 2013/05/23 T.Ono add
            strSQL.Append("TA.AUTO_FAXNO, ")       '自動FAX番号 2013/05/23 T.Ono add
            strSQL.Append("TA.AUTO_KBN, ")         '自動送信区分 2013/05/23 T.Ono add
            strSQL.Append("TA.AUTO_ZERO_FLG, ")    'ゼロ件送信フラグ 2013/05/23 T.Ono add
            strSQL.Append("TA.AUTO_FAXNM ")        '自動FAX送信名 2013/07/22 T.Ono add
        Else
        ...
        End If
        SqlParamC.fncSetParam("KBN", True, "3") 'hdnTANKBN.Value 1:ＪＡ担当者　2:監視センター担当者　3:出動会社担当者
    ...
    '▼▼▼ 2013/05/23 T.Ono add 顧客単位登録機能追加 fncDataSelectを元に作成▼▼▼
    '******************************************************************************
    '* 入力キーによるデータの検索を行います。
    '* pintKbn　0:検索ボタン押下時データ出力
    '*        　1:新規ボタン押下時データカウント出力
    ...
    Private Function fncDataSelect2(ByVal pintkbn As Integer) As DataSet
        ...
        strSQL.Append("TA.AUTO_MAIL_PASS, ")   '自動FAX添付ﾌｧｲﾙﾊﾟｽﾜｰﾄﾞ
        strSQL.Append("TA.AUTO_FAXNO, ")       '自動FAX番号
        strSQL.Append("TA.AUTO_KBN, ")         '自動送信区分
        strSQL.Append("TA.AUTO_ZERO_FLG, ")    'ゼロ件送信フラグ
        strSQL.Append("TA.AUTO_FAXNM ")        '自動FAX送信名
        ...
        If pintkbn = 0 Then 
            strSQL.Append(" ORDER BY TO_NUMBER(TANCD) ")
        End If
        SqlParamC.fncSetParam("KBN", True, "3") '1:監視センター担当者　2:出動会社担当者 3:JA支所担当者
        If hdnCODE.Value.Length > 0 Then
            SqlParamC.fncSetParam("CODE", True, hdnCODE.Value)
        End If
        If hdnKURACD.Value.Length > 0 Then
    ...
    Private Sub btnFileUpload_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnFileUpload.Click
        ...
        Dim sSaveFileKey As String 'ファイル保存時に頭に付けるキー（ｸﾗｲｱﾝﾄｺｰﾄﾞ＋JAｺｰﾄﾞ）
        ...
        Try
            ...
            If (uploadFile.FileName <> "") Then
                ...
                'sSaveFileKey = hdnKURACD.Value & "_" & hdnCODE.Value & "_" 'クライアントコード + JAコード
                If Trim(txtUSER_CD_FROM.Text) <> "" Then
                    sSaveFileKey = hdnKURACD.Value & "_" & hdnCODE.Value & "_" & Trim(txtUSER_CD_FROM.Text) & "_" 'クライアントコード + JAコード + お客様コード
                Else
                    sSaveFileKey = hdnKURACD.Value & "_" & hdnCODE.Value & "_" & "X" & "_" 'クライアントコード + JAコード + X お客様コードありと区別するため"X"をつける
                End If
    ...
    Private Sub btnFileDelete1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnFileDelete1.Click
        ...
        Dim sSaveFileName As String   '実際の元ファイル名（拡張子あり）  001_999_テストファイル.xls
        ...
        'sSaveFileName = hdnKURACD.Value.Trim & "_" & hdnCODE.Value.Trim & "_" & sSaveFileNameR 'ファイル名を、ｸﾗｲｱﾝﾄｺｰﾄﾞ＋ｺｰﾄﾞ＋ファイル名に変換
        sSaveFileName = hdnFileKey.Text & sSaveFileNameR 'ファイル名を、ｸﾗｲｱﾝﾄｺｰﾄﾞ＋ｺｰﾄﾞ＋ファイル名に変換

        If fncFileDelete(sSavePath, sSaveFileName, sSaveFileNameR) Then '成功？
            fncSearchAndSetFileName12() 'ファイル名だけ再表示
        End If
    End Sub
    ...
    Private Function fncFileDelete(ByVal sSavePath As String, ByVal sSaveFileName As String, ByVal sSaveFileNameR As String) As Boolean
        ...
        Try
            'ファイル存在チェック
            'If sSaveFileName.Length <= 0 Then 'ファイル存在しない？              '2012/04/20 NEC ou Del
            If sSaveFileNameR.Length <= 0 Then 'ファイル存在しない？              '2012/04/20 NEC ou Add
    ...
    Private Function fncFileDownload(ByVal sSavePath As String, ByVal sSaveFileName As String, ByVal sSaveFileNameR As String) As String
        ...
        Try
            ...
            If System.IO.File.Exists(fpath) Then
                ...
                'dt = Convert.FromBase64String(strRec) 'Webサービスの戻り値（BASE64のテキスト）をバイトデータに変換する
                'HttpHeaderC.mDownLoad(Response, sSaveFileNameS & ".xls") 'ファイル名は、HttpUtility.UrlEncodeメソッドを使用してSJISにエンコードする必要がある
                'Response.BinaryWrite(dt) 'ファイル送信
                'Response.End()
