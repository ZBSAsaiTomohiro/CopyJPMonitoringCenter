...
' 2021/10/01 Saka 2021年度監視改善③監視対応中のデータは自動対応処理で落とさない(感震器遮断の自動対応有効化は、地震などの直後に設定することに対する改善)
' 2021/10/01 Saka 2021年度監視改善④の、1時間前あるいは1日前に同一顧客から警報が発生していたら色を変えるという改善について、結局改善しないとなったため、ロジックが残っている(いつか復活する？)
...
#End Region
            ...
            Select Case KEJUKJAG00C.pExecFlag
                Case "DATACHECK"
                    ...
                    If CLng(KEJUKJAG00C.pDataCount) <> intDataCount Then
                        ...
                        '表示している警報の処理№を保存 2013/12/18 T.Ono add 監視改善2013
                        '2019/11/01 w.ganeko 2019監視改善 No3,4
                        'For intRow = 0 To 2
                        For intRow = 0 To 5
                        ...
                        Case "1"
                            ...
                            Else
                                ...
                                If dbData.Tables(0).Rows.Count = 0 Then                                 '2017/05/30 w.ganeko mod
                                    ...
                                    '表示中の処理№（画面上）または、より古い警報がなかった場合、最終ページに遷移
                ...
                Case "DATARENEW"
                    ...
                    If KEJUKJAG00C.pCtlFlg = "KEJUTAI" AndAlso KEJUKJAG00C.pSERIAL <> "" AndAlso CLng(KEJUKJAG00C.pDataCount) = 0 Then
                        '(対応入力用ボタンから遷移or対応入力画面から遷移) AND 処理№を保持している AND 前回データ総数=0
                        ...
                        If dbData.Tables(0).Rows.Count = 0 Then                                     '2017/05/30 w.ganeko mod
                            'If Convert.ToString(dbData.Tables(0).Rows(0).Item(0)) = "XYZ" Then     '2017/05/30 w.ganeko mod
                            '選択警報の処理№または、より古い警報がなかった場合、最終ページに遷移
                            If KEJUKJAG00C.pChkMishori = "1" Then
                                '未処理のみにチェックあり
                                '2019/11/01 w.ganeko 2019監視改善 No3,4 start
                                'If intDoReactionCoun < 4 Then
    ...
    Private Sub fncDataSet(ByRef pdbData As DataSet, ByVal psreCtl As String)
        ...
        'Dim dbData3 As DataSet     '2021/10/01 saka 2021年度監視改善④の1時間前に同一顧客警報で対応してたら色変えは対応しないため、残しておくが削除 
        ...
        If pdbData.Tables(0).Rows.Count = 0 Then                                  '2017/05/30 w.ganeko mod start
            ...
            For intRow = 0 To 5
                ...
                'strMsg.Append("Form1.txt" & intRow + 1 & "KURACD.className = 'c-rNM';")  '2021/10/01 saka 2021年度監視改善④の1時間前に同一顧客警報で対応してたら色変えは対応しないため、残しておくが削除
        ...
        Else
            ...
            dbData2 = fncDupUser()
            'dbData3 = fncHourUser()                     '2021/10/01 saka 2021年度監視改善④の1時間前に同一顧客警報で対応してたら色変えは対応しないため、残しておくが削除
            ...
            For intRow = 0 To 5
                ...
                Else
                    ...
                    Else
                        ...
                        End If
                        '2019/11/01 W.GANEKO 2019改善開発 No3,4 end
                        'If dbData3.Tables(0).Rows.Count > 0 Then                '2021/10/01 saka 2021年度監視改善④の1時間前に同一顧客警報で対応してたら色変えは対応しないため、残しておくが削除
    ...
    Private Function getSqlExistsAutoTaiou(ByVal pstrSYORI_SERIAL As String) As String
        ...
        strSQL.Append("  AND KEI.ROC_FRG IS NULL ") '2021/10/01 2021年度監視改善③ sakaUPD 対応中の警報は自動対応で落とさない
    ...
    '処理№から、ROWNUMIDを取得 kbn 1:その警報または、より古い警報 2:古い警報 3:新しい警報
    '戻り値：
    '**********************************************************
    ...
    End Function
    '***************************************************
    ' 2021/10/01 sakaADD 2021年度監視改善④1日以内に警報発生していたら色を変える、は改善しないとなったため、ロジックは残しておくが削除
    ...
    'End Function        '←2021年度監視改善④1日以内に同一顧客から警報対応していたら色を変える2021/10/01
End Class
