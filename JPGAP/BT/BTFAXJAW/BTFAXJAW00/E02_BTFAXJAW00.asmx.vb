...
#End Region
    ...
    <WebMethod()> Public Function mExcel( _
        ...
        Try
            ...
            strSQL.Append("                        AND JAS.HAN_CD = TAI.ACBCD  ")
            strSQL.Append("                        AND TAI.FAXKBN = '2' ")
            strSQL.Append("                        AND TAI.TMSKB  = '2' ")
            strSQL.Append(strWHERE_TAIOU.ToString)
            strSQL.Append("                ) ")
            strSQL.Append("                OR  ")
            ...
            strSQL.Append("    SELECT  ")
            strSQL.Append("        JAS.AUTO_KUBUN,  ")
            strSQL.Append("        DECODE(JAS.AUTO_KUBUN, 1, RTRIM(JAS.TELA_FAX1) || RTRIM(TELB_FAX1),'') AS AUTO_FAX,  ") ' 2010/05/24 T.Watabe add
    ...
    Private Function mExcelOut( _
        ...
        Dim ds As New DataSet
        Dim dr As DataRow
        Dim strFAXNO2 As String = "" ' 2010/05/24 T.Watabe add
        Dim strSendFaxNo As String = "" ' 2010/05/24 T.Watabe add
        ...
        Try
            ...
            strSQL.Append("    ,RTRIM(JAS.TELA_FAX1) || RTRIM(TELB_FAX1) AS AUTO_FAX2 ") ' 2010/05/24 T.Watabe add
            ...
            If ds.Tables(0).Rows.Count = 0 Then  '//データが存在しない？
                ...
                If pstrAUTO = "1" Then
                    ...
                    strSQL.Append("    AND JAS.AUTO_KUBUN  = '1' ")
                    strSQL.Append("    AND (JAS.AUTO_FAX = '" & pstrFAXNO & "' OR RTRIM(JAS.TELA_FAX1) || RTRIM(TELB_FAX1) = '" & pstrFAXNO & "')")
                Else
                    strSQL.Append("    AND JAS.AUTO_KUBUN  = '2' AND JAS.AUTO_MAIL    = '" & pstrMAILAD & "' ")
                End If
                ...
                strSendFaxNo = pstrFAXNO ' 2010/05/24 T.Watabe add

                If dsInfo.Tables(0).Rows.Count > 0 Then  '//データが存在する？
                    drInfo = dsInfo.Tables(0).Rows(0)
                    kenName = Convert.ToString(drInfo.Item("KEN_NAME"))
                    centerName = Convert.ToString(drInfo.Item("NAME"))
                    jalpTel = Convert.ToString(drInfo.Item("TEL"))

                    '' 2010/05/24 T.Watabe add
                    'strFAXNO2 = Convert.ToString(drInfo.Item("AUTO_FAX2"))
                    'If strFAXNO2.Length > 0 Then
                    '    strSendFaxNo += ";" & strFAXNO2
                    'End If
                ...
                If pstrAUTO = "1" Then
                    'ExcelC.pCellVal(1, "colspan=6") = "送信先FAX番号：" & pstrFAXNO ' 2010/05/24 T.Watabe edit
                    ExcelC.pCellVal(1, "colspan=6") = "送信先FAX番号：" & strSendFaxNo
                Else
                ...
                '8行目
                ExcelC.pCellStyle(1) = "border-style:none"
                ExcelC.pCellVal(1, "colspan=6") = "本日対応０件"
                ExcelC.mWriteLine("")   '行をファイルに書き込む
            ...
            Else
                ...
                For Each dr In ds.Tables(0).Rows
                    ...
                    If pstrAUTO = "1" Then
                        ...
                        ' 2010/05/24 T.Watabe add
                        strSendFaxNo = pstrFAXNO
                        'strFAXNO2 = Convert.ToString(dr.Item("AUTO_FAX2"))
                        'If strFAXNO2.Length > 0 Then
                        '    strSendFaxNo += ";" & strFAXNO2
                        'End If
            ...
            If pstrAUTO = "1" Then '1:fax
                ...
                'compressC.p_NihongoFileName = pstrSESSION & pstrAUTO & pstrFAXNO & ".xls" ' 2010/05/24 T.Watabe edit
                compressC.p_NihongoFileName = pstrSESSION & pstrAUTO & strSendFaxNo & ".xls"
