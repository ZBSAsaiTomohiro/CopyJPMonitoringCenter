...
'***********************************************
' 変更履歴
' 2013/06/26 W.GANEKO BTFAXJBWからのコピー新規
' 2013/09/09 T.Watabe JAｺｰﾄﾞ３桁固定で参照していたための不具合改修
' 2013/09/25 T.Watabe S05_AUTOFAXLOGDB 自動FAXのログを記録する。
' 2013/10/01 T.Watabe edit デバッグ時は帳票の右上作成日を指定した日付にする。
' 2014/06/17 T.Watabe 別圧縮DLLのテスト
' 2015/01/22 T.Watabe JA支所コードfrom－to、FAX番号orメールアドレスの指定を追加
' 2015/01/22 T.Watabe 警報コード1がNULLの場合は、警報名称1を表示しないように変更。同様に警報コード2から6も対応。
...
<System.Web.Services.WebService(Namespace:="http://tempuri.org/JPGAP.BTFAXJDW00/BTFAXJDW00")> _
Public Class BTFAXJDW00
...
#End Region
    ...
    <WebMethod()> Public Function mExcel3( _
        ...
        Dim strWHERE_JAS As New StringBuilder("") '2015/01/22 T.Watabe add
        ...
        '2015/01/22 T.Watabe add
        'JA支所FROM-TOの条件追加
        If pstrJASCD_F.Length > 0 Then
            strWHERE_TAIOU.Append("  AND TAI.ACBCD >= '" & pstrJASCD_F & "' " & vbCrLf)
            strWHERE_JAS.Append("  AND A.CODE >= '" & pstrJASCD_F & "' " & vbCrLf)
        End If
        If pstrJASCD_T.Length > 0 Then
            strWHERE_TAIOU.Append("  AND TAI.ACBCD <= '" & pstrJASCD_T & "' " & vbCrLf)
            strWHERE_JAS.Append("  AND A.CODE <= '" & pstrJASCD_T & "' " & vbCrLf)
        End If
        ...
        '2015/01/22 T.Watabe add
        'FAX番号orﾒｰﾙｱﾄﾞﾚｽの条件追加
        If strMAIL.Length > 0 Then strWHERE_CLI.Append("  AND A.AUTO_MAIL = '" & strMAIL & "' " & vbCrLf)
        If strFAXNO.Length > 0 Then strWHERE_CLI.Append("  AND A.AUTO_FAXNO = '" & strFAXNO & "' " & vbCrLf)
        If strMAIL.Length > 0 Then strWHERE_FAXMAIL.Append("  AND AUTO_MAIL = '" & strMAIL & "' " & vbCrLf)
        If strFAXNO.Length > 0 Then strWHERE_FAXMAIL.Append("  AND AUTO_FAX = '" & strFAXNO & "' " & vbCrLf)
        ...
        Try
            ...
            If True Then
                If pstrSEND_KBN = "1" Then '送信区分 1:販売所(JA・支所)/2:ｸﾗｲｱﾝﾄ
                    ...
                    strSQL.Append("    WHERE " & vbCrLf)
                    strSQL.Append("              TANTO2.AUTO_KBN IN ('" & strFAXKBN & "', '" & strBoth & "') " & vbCrLf)
                    strSQL.Append("          AND TANTO2.AUTO_FAXNO IS NOT NULL " & vbCrLf)
                    strSQL.Append("          AND TANTO2.AUTO_ZERO_FLG ='1' " & vbCrLf)
                    ...
                    strSQL.Append("          TANTO2 " & vbCrLf)
                    strSQL.Append("    WHERE " & vbCrLf)
                    strSQL.Append("              TANTO2.AUTO_KBN IN ('" & strMAILKBN & "', '" & strBoth & "') " & vbCrLf)
    ...
    <WebMethod()> Public Function dispBatchLog(ByRef batchLog As String) As Boolean
        ...
        Try
            ...
            sql.Append("WHERE PROC_ID = 'BTFAXJCE00' ")
