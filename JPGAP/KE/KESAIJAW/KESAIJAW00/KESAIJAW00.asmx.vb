...
' 2021/10/01 2021年度監視改善⑥SakaUPD 未処理（画面では"継続対応中"）件数を出力するための期間区分を追加＋赤文字注意事項出力
...
#End Region
    ...
    <WebMethod()> Public Function mExcel(
                                        ...
                                        ByVal pstrTSADNM As String,     '2021年度監視改善⑥sakaUPD 2021/10/01
                                        ByVal pstrKIKANKBN As String    '←2021年度監視改善⑥sakaADD期間区分を追加（元の対応完了日と受信日（未処理出力となる））
                                        ) As String
        ...
        Dim styleRed As Icellstyle  '2021年度監視改善⑥2021/10/01
        Dim redFont As IFont        '2021年度監視改善⑥赤文字フォント2021/10/01
        ...
        '帳票出力項目の取得用SQL文セット-------------------
        Try
            '帳票1種類のまま If pstrKIKANKBN = "1" Then                                     '2021年度監視改善⑥sakaUPDﾘｱﾙﾀｲﾑの未処理出力(受信日のラジオを増やし、受信日の場合は感震器遮断用未処理も出力)2021/10/01
            '帳票出力項目の取得用SQL文セット
            ...
            'ElseIf pstrKIKANKBN = "2" Then                                  '←受信日選択時(未処理感震器出力とする)sakaADD '2021年度監視改善⑥未処理感震出力'2021/10/01
            ...
            'pstrTSADNM
            '))
            'End If                                                           '←帳票出力の追加はここまで2021年度監視改善⑥'2021/10/01
            ...
            'If pstrKIKANKBN = "1" Then                '←2021年度監視改善⑥未処理感震器出力sakaADD2021/10/01
            ...
            '帳票一種類のままと決まったためコメント化 If pstrKIKANKBN = "1" Then         '←2021年度監視改善⑥未処理感震器出力2021/10/01
            'sheet1.AddMergedRegion(New NPOI.SS.Util.CellRangeAddress(4, 5, 9, 9))
            'Else
            sheet1.AddMergedRegion(New NPOI.SS.Util.CellRangeAddress(4, 5, 11, 11))      '←2021年度監視改善⑥未対応枠を増やしたため2列後ろへ
            'End If

            'sheet1.createcellstyle(New npoi.ss.util.cellstyle)
            'sheet1.font
            ...
            style1.ShrinkToFit = True         '2021年度監視改善2021/10/01⑥未処理感震器出力ヘッダの文字サイズセル枠に合わせてもらう
            ...
            '2021年度監視改善⑥赤文字注意事項出力2021/10/01 セッティングStart↓↓
            styleRed = book.CreateCellStyle()
            styleRed.VerticalAlignment = NPOI.SS.UserModel.VerticalAlignment.Center '文字位置の上下を中心にする
            'styleRed.Alignment = HorizontalAlignment.CenterSelection
            'styleRed.BorderLeft = NPOI.SS.UserModel.BorderStyle.Thin 'スタイルそのものが適用されているか、ためしに付けたもの。本来不要

            redFont = book.CreateFont()
            redFont.FontName = "ＭＳ Ｐゴシック" 'フォント名指定できる
            redFont.Color = 2           '文字色 2が赤
            redFont.Boldweight = FontBoldWeight.Bold 'フォント太さ変更、太字
            'redFont.FontHeight = 25  'フォントサイズ変更 
            'redFont.Boldweight = True 'こっちはエラーになるのでコメントアウト
            styleRed.SetFont(redFont) 'フォント設定を、スタイル内のフォント設定に反映
            '2021年度監視改善⑥赤文字注意事項出力2021/10/01 セッティングはここまで↑↑

            'ヘッダ
            If iRow = 0 Then
                'タイトル
                rows = sheet1.CreateRow(0)
                '帳票1種類のまま If pstrKIKANKBN = "1" Then                           '2021年度監視改善⑥未処理感震器出力sakaADD2021/10/01
                ...
                If pstrTrgFrom.Length > 0 Then
                    ...
                    End If
                    If pstrKIKANKBN = "1" Then                                                       '←2021年度監視改善⑥未処理感震器出力sakaADD'2021/10/01
                        strHedInfo1 = strHedInfo1 & "・対応完了日"                                   '←2021年度監視改善⑥元々の帳票では対応完了日
                    Else
                        strHedInfo1 = strHedInfo1 & "・受信日時（未処理データも出力）"  '←2021年度監視改善⑥未処理感震器出力akaADD'2021/10/01
                    End If
                End If
                ...
                End If
                strHedInfo2 = strHedInfo2 & "、帳票出力日時：" & DateTime.Now.ToString         '←2021年度監視改善⑥2021/10/01帳票出力日時を出力

                rows = sheet1.CreateRow(2)
                rows.CreateCell(1).SetCellValue(strHedInfo2)

                '2021年度監視改善⑥2021/10/01赤文字注意事項出力
                rows = sheet1.createrow(3)
                rows.createcell(3).setcellvalue("※継続対応中は、連絡がつかないお客様を含む")
                rows.GetCell(3).CellStyle = styleRed       '赤文字Font設定を含むスタイルをここで適用
                ...
                '帳票1種類のまま If pstrKIKANKBN = "1" Then                                     '←2021年度監視改善⑥未処理感震器出力sakaADD'2021/10/01
                ...
                'End If                                                    '←この条件はここまで2021年度監視改善⑥未処理感震器出力sakaADD'2021/10/01
                iRow = 5
                rows = sheet1.CreateRow(iRow)
                'クライアント
                rows.CreateCell(1)
            ...
            'sheet1.CreateRow(iRow).CreateCell(1).SetCellValue("※赤文字のテスト中です")  '2021年度監視改善⑥最後に出力する場合（使用しないためコメント化）
            'sheet1.GetRow(iRow).GetCell(1).CellStyle = styleRed '赤文字Font設定を含むスタイルを、ここに適用させる。
    ...
    End Function
    '******************************************************************************
    '*　概　要:ＳＥＬＥＣＴ文を作成　'2021/10/01　2021年度監視改善⑥の未処理件数出力によりコメント化
    '*　備　考:対応ＤＢ取得
    '******************************************************************************
    ...
    Public Function fncMakeSelect(ByVal pstrKURACDFrom As String,
        ...
        '日時の検索条件
        If pstrKIKANKBN = "1" Then                   '←2021年度監視改善⑥未処理感震器出力sakaADD受信日
            '対応完了日
            If pstrTrgTimeFrom.Length = 0 AndAlso pstrTrgTimeTo.Length = 0 Then
                strWHE.Append("AND    SYOYMD BETWEEN :TRGDATE_FROM AND :TRGDATE_TO ")
            ElseIf pstrTrgTimeFrom.Length > 0 AndAlso pstrTrgTimeTo.Length > 0 Then
        ...
        Else '受信日
            ...
            ElseIf pstrTrgTimeFrom.Length > 0 AndAlso pstrTrgTimeTo.Length > 0 Then
                strWHE.Append("AND    HATYMD || HATTIME BETWEEN :TRGDATE_FROM || :TRGTIME_FROM AND :TRGDATE_TO || :TRGTIME_TO ") '←2021年度監視改善⑥未処理感震器出力sakaADD受信日時間(HATYMD||HATTIME)'2021/10/01
            End If
        ...
        strSQL.Append("       , Z.JACD ")
        strSQL.Append("       , Z.JACD || ':' || Z.JANM AS JADISP ")
        strSQL.Append("       , SUM(DECODE(HATKBN || TAIOKBN || TMSKB,'122',CNT,0)) AS C01 ")            '←2021/10/01 2021年度監視改善⑥Start
        ...
        strSQL.Append("       , SUM(DECODE(TAIOKBN,'1',CNT,'2',CNT,0)) AS SUMA ")                         '←2021/10/01 2021年度監視改善⑥End
        strSQL.Append("FROM   Z LEFT JOIN CLIMAS C ON Z.KURACD = C.CLI_CD ")
        strSQL.Append("GROUP BY Z.KURACD ")
        strSQL.Append("         , Z.JACD ")
        strSQL.Append("         , Z.JANM ")
        ...
        strSQL.Append("       , SUM(DECODE(HATKBN || TAIOKBN,'21',CNT,'22',CNT,0)) AS SUM2 ")
        strSQL.Append("       , SUM(DECODE(TAIOKBN,'1',CNT,'2',CNT,0)) AS SUMA ")                         '←2021/10/01 2021年度監視改善⑥End
        '        strSQL.Append("       , SUM(DECODE(HATKBN || TAIOKBN || TMSKB,'122',CNT,0)) AS C01 ") '←2021/10/01 2021年度監視改善⑥によりコメント化
