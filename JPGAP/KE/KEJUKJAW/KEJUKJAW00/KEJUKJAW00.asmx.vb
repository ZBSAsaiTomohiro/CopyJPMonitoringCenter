...
'2021/10/01 Saka 2021年度監視改善③自動対応で対応中の警報は落とさない（感震器遮断警報を自動対応にする作業を地震直後の運用中に更新するため）
'2021/10/01 Saka 2021年度監視改善⑥の感震器遮断の未処理警報を出力するときに既存バグで対応済みでないのに対応完了日(SYOYMD)に日付がセットされるのを防ぐ
'2021/10/01 Saka 2021年度監視改善⑦電話番号14ケタ化対応（市外4桁、市内10けた）で、JUTEL2のハイフンセット処理を無効にして、市内10桁をハイフンなしでそのまま10桁セットとする
'2023/01/06 Y.ARAKAKI 2022更改No⑥ _帳票JMコード表示追加対応  
    ...
    <WebMethod()> Public Function mSet_AutoTaiou(ByVal pstrSYORI_SERIAL As String) As String
        ...
        Try
            ...
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                ...
                ' 2023/01/04 ADD Y.ARAKAKI 2022更改No⑥ _帳票JMコード表示追加対応 自動対応データ登録でもJM和名を登録できるよう修正（共通処理だった為併せて修正）
...
[Continue]:
    ...
    Private Function getAutoKeiho(ByVal pstrSyoriNo As String, ByVal strPar() As String) As String
        ...
        strSQL.Append(",NVL(K3.GROUPNM, NVL(K2.GROUPNM, NVL(K1.GROUPNM, ''))) AS JMNAME ") ' 2023/01/04 ADD Y.ARAKAKI 2022更改No⑥ _帳票JMコード表示追加対応 JM和名（グループCD取得できれば必ず取得できるはず）
        '==========================================
        'FROM句
        '==========================================
        ...
        strSQL.Append("  AND K1.GROUPNM(+) IS NOT NULL  ") ' 2023/01/04 ADD Y.ARAKAKI 2022更改No⑥ _帳票JMコード表示追加対応 
        'ユーザー範囲
        ...
        strSQL.Append("  AND K2.GROUPNM(+) IS NOT NULL  ") ' 2023/01/04 ADD Y.ARAKAKI 2022更改No⑥ _帳票JMコード表示追加対応 
        'ユーザー個別
        ...
        strSQL.Append("  AND K3.GROUPNM(+) IS NOT NULL  ") ' 2023/01/04 ADD Y.ARAKAKI 2022更改No⑥ _帳票JMコード表示追加対応 
        '2016/12/26 H.Mori add 2016改善開発 No4-6 END
    ...
    Private Sub editTaiouData(ByVal pRow As DataRow, ByVal cdb As CDB, ByRef dto As KEJUKJAW00DTO.D20TaiouDto, ByVal atDto As KEJUKJAW00DTO.AutoTaiouDto)
        ...
        Try
            ...
            Else
                ...
                'strTemp = Convert.ToString(pRow.Item("JUTEL2"))   '←2021/10/01 2021年度監視改善⑦sakaDEL結線電話番号14ケタ化対応（ハイフン削除で市内10桁→JUTEL2（10桁））
                'If strTemp.Length > 4 Then
                'strTemp = strTemp.Substring(0, strTemp.Length - 4) & "-" & strTemp.Substring(strTemp.Length - 4, 4)
                'End If
                'dto.JUTEL2 = strTemp
                dto.JUTEL2 = Convert.ToString(pRow.Item("JUTEL2")) '←2021/10/01 2021年度監視改善⑦sakaADD結線電話番号14ケタ化対応（ハイフン削除で市内10桁→JUTEL2（10桁））
                '連絡先
                ...
                Dim strTMSKB As String = Convert.ToString(atDto.tmskb)               '←2021/10/01 Start 2021年度監視改善⑥に関係し、自動対応で処理区分が2:処理済みでない場合は対応完了日(SYOYMD)は空欄とする

                If strTMSKB = "2" Then                                               '←2021/10/01 既存バグの修正となる、⑥で自動対応で未処理で登録されたデータを対応完了日などで検索させるが、日付が入っていてヒットしてしまう事が判明したため
                ...
                ' 2023/01/04 ADD START Y.ARAKAKI 2022更改No⑥ _帳票JMコード表示追加対応
                'JM和名
                dto.JMNAME = Convert.ToString(pRow.Item("JMNAME"))
                ' 2023/01/04 ADD END   Y.ARAKAKI 2022更改No⑥ _帳票JMコード表示追加対応
            End If
        Catch ex As Exception
            Throw ex
        End Try
